...
//         page.drawText(`PRODUCT: ${room.productName}`, { x: 50, y: yPosition, size: 12, font: boldFont, color: PRIMARY_COLOR });
//         yPosition -= 15;
//         drawTableHeaders();
...
export const generateCutlistPDF = async (cutlistData: any, orgName: string, COMPANY_LOGO: string) => {
    ...
    // 1. Updated Column Widths to accommodate long names
    const colWidths = [40, 60, 55, 65, 80, 80, 110, 110, 135, 110, 110, 135];
    const headers = [
        "S No", "Height (mm)", "Width (mm)", "Plywood Thickness", "Inner Laminate Thickness", "Outer Laminate Thickness",
        "Back Side Brand", "Back Name/Code", "Back Image",
        "Front Side Brand", "Front Name/Code", "Front Image"
    ];
    ...
    const drawTableHeaders = () => {
        const headerHeight = 35; // Standard single height
    ...
    for (const room of cutlistData.rooms) {
        ...
        room.items.forEach((item: any, idx: number) => {
            ...
            // 1. Draw columns (Updated with separate h and w)
            const rowData = [
                String(idx + 1),
                `${item.h || 0}`, // Height
                `${item.w || 0}`, // Width
            ...
            // 2. Identify Image Column X (Adjust index because of new column)
            // const backImageX = 50 + colWidths.slice(0, 8).reduce((a, b) => a + b, 0);
            // 2. Identify Image Column X and move past it
            const backImageX = curX;
            curX += colWidths[8];
